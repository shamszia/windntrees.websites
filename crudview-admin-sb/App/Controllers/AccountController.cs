    ...
    public class AccountController : BasicController
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    IList<string> listRoles = await _userManager.GetRolesAsync(user);
                    foreach(var adminRole in applicationSettings.AdminRoles)
                    {
                        foreach(var userRole in listRoles)
                        {
                            if (adminRole.Equals(userRole, StringComparison.OrdinalIgnoreCase))
                            {
                                isAdminRole = true;
                                break;
                            }
                    ...
                    if (isAdminRole)
                    {
                        //if user is approved and have expiry date within sepcified time
                        if (user.IsApproved && (user.ExpiryDate == null ? true : (user.ExpiryDate > DateTime.Now)))
                        {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                //ApprovedBy,CreationDate,ExpiryDate,FirstName,ImageFile,IsApproved,LastName,Package,Sex,Title
                ...
                if (result.Succeeded)
                {
                    //await _userManager.AddToRoleAsync(user, "mngr_myaccount");
                    ...
                    RepositoryUserRoles.AddUserRole(user.Id, "mngr_myaccount");
                    ...
                    HttpContext.Session.SetString("roles", "mngr_myaccount");
