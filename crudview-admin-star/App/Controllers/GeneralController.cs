        ...
        public JsonResult SendEmail([FromBody] Email emailModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (!string.IsNullOrEmpty(recipientAddress))
                {
                    ...
                    {   
                        return GetObjectResultWithException(emailModel, "Exception", ex.Message);
                    }
                }
            }

            return GetObjectResultWithException(emailModel, "Form", SharedLibrary.Core.Resources.Global.StandardMessages.AlertErr);
        }
    }
}
