...
namespace DataAccess.Core.Repositories
{
    public class ConfigurationsRepository : EntityRepository<Configuration>
    {
        ...
        {
            var content = ((ApplicationContext)context).Configurations.Where(l => l.KeyParam == key).SingleOrDefault();
            if (content != null)
            {
        ...
        {
            List<GenericListString> contents = ((ApplicationContext)context).Configurations.OrderBy(l => l.KeyParam).ToList()
                    .Select(param =>
                    {
