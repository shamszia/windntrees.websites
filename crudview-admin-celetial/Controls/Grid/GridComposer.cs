...
namespace Controls.Core.Grid
{
    ...
    public class GridComposer
    {
        ...
        {
            string newFormAttributes = "data-bind=\"click: function(data, event) { resetForm(); }\" class='btn btn-primary btn-control-primary' data-toggle='modal' data-target='" + formId + "'";
            ...
            if (!modalForm)
            {
                newFormAttributes = "data-bind=\"click: function(data, event) { resetForm(); $('" + formId + "').dialog('open'); }\" class='btn btn-primary btn-control-primary'";
            ...
            string basicViews = "<section> " +
                                    ...
                                    "            <span class='input-group-btn'> " +
                                    "                <button data-bind='click: function() { list(1); }' class='btn btn-primary btn-control-primary' type='button' title='" + SharedLibrary.Core.Resources.Global.FormMessages.SearchD + "'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.Search + "</span></button> " +
                                    "            </span> " +
                                    "        </div> " +
                                    "    </div> " +
                                    "    <br /> " +
                                    ...
                                    "            <select class='form-control' data-bind='value: ListSize, event: {change: function() { list(CurrentList()); }}' style='width:auto;'> " +
                                    ...
                                    "                <li class='input-group-addon' data-bind='css: {disabled: CurrentList() == 1}'><a href = '#' data-bind='click: function(){ if (CurrentList() > 1) {list(CurrentList() - 1); } }'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.PrevPage + "</span></a></li> " +
                                    "                <li class='input-group-addon'><select data-bind=\"value: CurrentList, options: ListNavigator().getLists(), optionsText: 'Number', optionsValue:'Number', event: {change: function() { list(CurrentList()); }}\" style='width:auto;' class='form-control' /></li> " +
                                    "                <li class='input-group-addon' data-bind='css: {disabled: CurrentList() == ListNavigator().calculateTotalPages()}'><a href = '#' data-bind='click: function(){ if (CurrentList() <ListNavigator().calculateTotalPages()) {list(CurrentList() + 1); } }'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.NextPage + "</span></a></li> " +
                                    "            </ul> " +
                                    "        </div> " +
                                    "    </div> " +
                                    "</script> " +
        ...
        public static HtmlString composeStandardActions(string actionId, string formId, Boolean readOnly = false, string searchAction = null, string searchActionParams = null, bool modalForm = true, string newDialogAttributes = null, string actionContentId = null, string searchLink = null)
        {
            ...
            searchAction = searchAction == null ? "list" : searchAction;
            ...
            string standardActions = "<script type = 'text/html' id='" + actionId + "'> " +
                                        ...
                                        "                <button data-bind='click: function() { " + searchAction + " }' class='btn btn-primary btn-control-primary' type='button' title='" + SharedLibrary.Core.Resources.Global.FormMessages.SearchD + "'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.Search + "</span></button> " +
                                        "            </span> " +
                                        "        </div> " +
                                        "    </div> " +
                                        "</script> ";
        ...
        public static HtmlString composeStandardListing(string listingId, string formId, Boolean navLinks = false, string searchAction = null, string selectList = null, string nextList = null, string prevList = null, string postSelectionAction = null)
        {
            ...
            var navLinkSizeSelectList = string.Format("{0}({1});", "list", "CurrentList()", "");
            ...
            string standardListing = "   <script type = 'text/html' id='" + listingId + "'> " +
                                        ...
                                        "            <select class='page-list-size' data-bind=\"value: ListSize, event: {change: function() { " + (navLinks ? navLinkSizeSelectList : selectAction) + " }}\" id='form-field-select-1' style='width:auto;'> " +
                                        "                <option value = '10'> 10 </option> " +
                                        "                <option value ='20'>20</option> " +
                                        "                <option value = '50'> 50 </option> " +
                                        "                <option value ='100'>100</option> " +
        ...
        public static HtmlString composeDetailActions(string formId)
        {
            string detailActions = "<section> " +
                                    ...
                                    "                <button data-bind='click: function() { resetForm(); }' class='btn btn-primary btn-control-primary' data-toggle='modal' data-target='" + formId + "' title='" + SharedLibrary.Core.Resources.Global.FormMessages.NewD + "'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.New + "</span></button> " +
                                    "            </span> " +
                                    "            <span class='input-group-btn' data-bind='visible: $parents[0].getContextualMasterKeyRecord() !== null'> " +
                                    "                <button data-bind=\"click: function() { resetDetailForm({ '_relatedrecordtype': 'level1-record-id', '_relatedrecordid': $parents[0].getContextualMasterKeyRecord().getKey() }); }\" class='btn btn-primary btn-control-primary' data-toggle='modal' data-target='" + formId + "' title='" + SharedLibrary.Core.Resources.Global.FormMessages.FieldNewDetailD + "'><span> " + SharedLibrary.Core.Resources.Global.FormMessages.FieldNewDetail + " </span></button> " +
                                    "            </span> " +
                                    "            <input class='form-control' data-bind='value: Keyword' type='text' placeholder='" + SharedLibrary.Core.Resources.Global.FormMessages.Keyword + "' /> " +
                                    "            <span class='input-group-btn' data-bind='visible: $parents[0].getContextualMasterKeyRecord() === null'> " +
                                    "                <button data-bind='click: function() { list(1); }' class='btn btn-primary btn-control-primary' type='button' title='" + SharedLibrary.Core.Resources.Global.FormMessages.SearchD + "'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.Search + "</span></button> " +
                                    "            </span> " +
                                    "            <span class='input-group-btn' data-bind='visible: $parents[0].getContextualMasterKeyRecord() !== null'> " +
                                    "                <button data-bind='click: function() { listDetail(1); }' class='btn btn-primary btn-control-primary' type='button' title='" + SharedLibrary.Core.Resources.Global.FormMessages.SearchD + "'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.Search + "</span></button> " +
                                    "            </span> " +
                                    "        </div> " +
                                    "    </div> " +
                                    "    <br /> " +
        ...
        public static HtmlString composeDetailListing(string formId)
        {
            string detailListing = "<section> " +
                                    ...
                                    "            <span class='input-group-addon'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.ListSize + "</span></span> " +
                                    "            <select class='form-control' data-bind='value: ListSize, event: {change: function() { list(CurrentList()); }}' id='form-field-select-1' style='width:auto;'> " +
                                    "                <option value = '10'> 10 </option> " +
                                    "                <option value ='20'>20</option> " +
                                    "                <option value = '50'> 50 </option> " +
                                    "                <option value ='100'>100</option> " +
                                    ...
                                    "            <ul class='gridlist' data-bind='if: ListNavigator().calculateTotalPages() > 1'> " +
                                    "                <li class='input-group-addon' data-bind='css: {disabled: CurrentList() == 1}'><a href = '#' data-bind='click: function(){ if (CurrentList() > 1) {list(CurrentList() - 1); } }'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.PrevPage + "</span></a></li> " +
                                    "                <li class='input-group-addon'><select class='form-control' data-bind=\"value: CurrentList, options: ListNavigator().getLists(), optionsText: 'Number', optionsValue:'Number', event: {change: function() { list(CurrentList()); }}\" id='form-field-select-paging' style='width:auto;' /></li> " +
                                    "                <li class='input-group-addon' data-bind='css: {disabled: CurrentList() == ListNavigator().calculateTotalPages()}'><a href = '#' data-bind='click: function(){ if (CurrentList() < ListNavigator().calculateTotalPages()) {list(CurrentList() + 1); } }'><span>" + SharedLibrary.Core.Resources.Global.FormMessages.NextPage + "</span></a></li> " +
                                    "            </ul> " +
                                    "        </div> " +
                                    "        <div class='input-group' data-bind='visible: $parents[0].getContextualMasterKeyRecord() !== null'> " +
